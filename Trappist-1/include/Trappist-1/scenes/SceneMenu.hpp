...
public:
	...
	static constexpr const char *STR_PLANET_NAMES[SceneMenu::IMG_PLANETS_SIZE] =
	{
		"Trappist-1b",
		"Trappist-1c",
		"Trappist-1d",
	...
	static constexpr const char *STR_PLANET_INFO[SceneMenu::IMG_PLANETS_SIZE] =
	{
...
private:
	...
	glm::vec2 baseBtnPosition;
	float baseBtnXOffset; // x button offset when hidden
	...
	float planetYPosition;
	float planetXPositions[SceneMenu::IMG_PLANETS_SIZE];
	float planetXOffset; // x planet offset when hidden
	
	std::size_t focusedPlanet; // index of the currently focussed planet (PLANET_SIZE if none focussed)
	glm::vec2 focusedPlanetPosition; // the position, on which the focused planet should be
